    ...
    <div class="card-body">
        <form action="{{route('admin.question.sopaletras.update', $sopaletra)}}" method="POST">
            ...
            <div class="form-group">
                ...
                @for($j=0; $j<(10 - count($sopaletra->indications)); $j++)
                    <input type="text" name="indicacionesedit[]" class="form-control" placeholder="Indicación {{count($sopaletra->indications)+$j+1}}" value="{{old('indication')}}">
                    <br>
                @endfor
